...
exports.main = async (event, context) => {
  ...
  if (event.Type === 'text'){
  await db.collection('NegotiationChat') // 替换为你的集合名称
  .where({
    Bargainer: db.command.all([event.AnotherOpenid, event.MyOpenid]) // 这里是你想要筛选的元素
  })
  .update({
...
const asd = await db.collection('User').where({openid:event.AnotherOpenid}).field({Bargainer:true}).get()
const Bargainerss = asd.data[0].Bargainer


let Bargainers = Bargainerss.map(item => {
  if (item.BargainerOpenid === event.MyOpenid) {
    // 修改属性b的值
    item.Unread += 1;
  }
  return item;
